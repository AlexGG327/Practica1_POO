...
#Mensaje a mandar
message_text = "Hola desde el ordenador de Alejandro"
...
def xor_hash(data): #Comunicador
    result = 0
    for char in data:
        result ^= char
    return result

def generate_hash(name, key): #Comunicador
...
def on_message(client, userdata, msg): #Comunicador
...
def decode_encrypted(mp): #Comunicador
        try:
...
def direct_message(destination_id): #Comunicador
...
def send_message(destination_id, message_text): #Comunicador
...
def send_traceroute(destination_id): #Comunicador
    if not client.is_connected():
        connect_mqtt()
...
def send_node_info(destination_id, want_response): #Comunicador
...
def send_position(destination_id): #Comunicador
    if client.is_connected():
...
def generate_mesh_packet(destination_id, encoded_message): #Comunicador
...
def encrypt_message(channel, key, mesh_packet, encoded_message): #Comunicador
    mesh_packet.channel = generate_hash(channel, key)
    key_bytes = base64.b64decode(key.encode('ascii'))
...
def disconnect_mqtt(): #Comunicador
    if debug: print("disconnect_mqtt")
    if client.is_connected():
        client.disconnect()

def on_connect(client, userdata, flags, reason_code, properties): #Comunicador
    set_topic()
    if client.is_connected():
...
def on_disconnect(client, userdata, flags, reason_code, properties): #Comunicador
